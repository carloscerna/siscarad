...
<div class="container-fluid py-4">
    <div class="card shadow-lg border-0 rounded-3">
        <div class="card-header bg-primary text-white py-3 d-flex align-items-center justify-content-between">
            <h5 class="card-title mb-0"><i class="fas fa-address-card me-2"></i> Actualización de Ficha del Estudiante (Literal B)</h5>
                            ...
                            <tr>
                                <td class="text-center">
                                    <div class="rounded-circle border bg-white shadow-sm d-inline-block" style="width: 50px; height: 50px; overflow: hidden;">
                                        ...
                                        @else
                                            <img src="{{ asset('img_central/' . ($al->codigo_genero == '02' ? 'avatar_femenino.png' : 'avatar_masculino.png')) }}" alt="Avatar" class="h-100 w-100" style="object-fit: cover;">
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="align-middle">
                                    <span class="d-block fw-semibold text-dark">{{ trim($al->grado_nombre ?? 'N/A') }}</span>
                                    <span class="badge badge-secondary px-2 py-1 mt-1">Sección: {{ trim($al->seccion_nombre ?? 'A') }}</span>
                                </td>
                                <td class="align-middle">
                                    <small class="d-block text-secondary mb-1"><i class="far fa-clock"></i> {{ trim($al->turno_nombre ?? 'Matutino') }}</small>
                                    <small class="text-muted text-truncate d-inline-block" style="max-width: 150px;">
                                        {{ $al->bachillerato_nombre ? trim($al->bachillerato_nombre) : 'Educación Básica' }}
                                    </small>
                                </td>
                                <td class="text-center align-middle">
                                    <a href="{{ route('ficha.edit', $al->id_alumno) }}" class="btn btn-sm btn-primary btn-block shadow-sm">
                                        <i class="fas fa-edit"></i> Actualizar Ficha
                                    </a>
                                </td>
                            </tr>
                        @endforeach
